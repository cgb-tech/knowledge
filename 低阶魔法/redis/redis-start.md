...
下载redis的压缩包  wget http://download.redis.io/releases/redis-5.0.5.tar.gz
...
自己在配置redis的时候 需要把redis中的密码给设置上（requirepass）（默认没有）

在启动的时候找到 桌面上的redis目录，在src下使用`./redis-cli -h 127.0.0.1 -p 6379 shutdown`停止redis服务

启动的时候使用`./redis-server ../redis.conf`来启动
...
Redis 列表是简单的字符串列表，按照插入顺序排序。你可以添加一个元素到列表的头部（左边）或者尾部（右边）。（链表）
...
![1564038924917](C:\Users\888\AppData\Roaming\Typora\typora-user-images\1564038924917.png)
